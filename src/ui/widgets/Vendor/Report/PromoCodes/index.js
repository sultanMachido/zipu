...
import {
	createPromoCode,
	fetchAllPromoCodes
} from '../../../../redux/actions/promoCodes/promoCodes.actions';
...
const PromoCodes = (props) => {
	...
	return (
		<div className="promoCodesWrapper">
			<Form form={form} onFinish={handleCreatePromoCode} hideRequiredMark layout="vertical">
				<div className="main">
					<Row gutter={0}>
						<Col span={12}>
								...
								<Row gutter={30}>
									<Col span={12}>
										<Form.Item
											...
											hasFeedback
											validateTrigger={['onChange', 'onBlur']}
											rules={[{ required: true, message: 'Please enter the discount type' }]}>
											<Select
									...
									<Col span={24}>
										<div className="btnWrapper">
											{SubmitButton(
												'ADD PROMO CODE',
												null,
												props?.promoCodes?.createPromoCodeLoading
											)}
										</div>
									</Col>
								</Row>
										...
										({
											...
											const amount =
												discount_type === 'amount'
													? `NGN ${custom_amount.toLocaleString()}`
													: discount_type === 'amount'
													? `${discount}%`
													: 'NGN 0';
											return (
												<div className="promoHistoryItem">
													...
													<p>Code: {code}</p>
													<p>
														{amount} valid from {moment(start_date).format('LL')} to{' '}
