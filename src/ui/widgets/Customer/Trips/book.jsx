...
import { getTrip } from 'redux/actions/trips/trips.actions';
...
const BookTrip = ({ trips: { trip, fetchTripLoading }, ...props }) => {
	useEffect(() => {
		loadTripDetails();
	}, []);
	...
	const loadTripDetails = async () => {
		...
		try {
			let request = await props.getTrip(tripId);
		} catch (err) {
			message.error(err);
		}
	};
				...
				{fetchTripLoading ? (
					...
					<Fragment>
						<View className={`${styles('hero--header')} ${styles('search-tab')}`}>
							<button onClick={() => props.history.goBack()} className={`${styles('btn--return')}`}>
									...
									<View className={`tsCol bordered ${styles('booking-aside-item')}`}>
										...
										<div className="tsrow justify-start mb-xs">
											<p className="body-2-sm grey-dark">
												In some cases, additional local taxes and fees may be due at the counter.
												Please see the reservation terms and conditions for more information.
											</p>
										</div>
