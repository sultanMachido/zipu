...
import { verifyEmailLoading, verifyEmailError, clearVerifyEmailErrors, verifyEmailSuccess } from './verifyEmail.actionsCreators';
...
export const verifyUserEmail = (otp) => async (dispatch) => {
  ...
  try {
    const verifyEmailRequest = await APIService.post('/verify-transco-email', { otp_code: otpCode });
