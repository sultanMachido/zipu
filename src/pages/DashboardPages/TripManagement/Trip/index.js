  ...
  return (
    <div className="tripWrapper">
      ...
      <div className="separator">
        <div
          className={`circle ${props?.record?.tripStatus === 'completed' ? "completed" : "notCompleted"}`}>
        </div>
        <div
          className={`dottedLine ${props?.record?.tripStatus === 'completed' ? "dottedCompleted" : "dottedNotCompleted"}`}>
        </div>
      </div>
      <div className="tripDetailsWrapper">
        <div className="tripDetails">
          <h5>{props?.record?.startingPoint} - {props?.record?.destination}</h5>
