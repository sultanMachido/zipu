	...
	const onFinishBusinessDetails = async values => {
		try {
			...
			if (tryBusinessDetails.businessDetailsStatus) {
				message.success({
					content: tryBusinessDetails.message,
					key: messageKey,
					duration: 10
				});
				query.set('step', 6);
			...
			<div className="businessDetailsForm">
				...
				>
					<div className="permitSubmitInputWrapper">
						<p>Comapny Details</p>
						{TranscoNameField(inputSize, true, 'Name of Transport Service')}
						{NumberOfStaffField(inputSize, true, 'Number of Employees')}
					...
					<div className="btnWrapper">
						{SubmitButton(
							'SUBMIT',
							null,
							props?.businessDetails?.businessDetailsLoading
						)}
					</div>
				</Form>
			</div>
